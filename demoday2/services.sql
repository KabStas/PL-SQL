...
begin
    ...
    v_result := kabenyk_st.pkg_specialties.is_gender_matched (
        p_id_patient => p_id_patient,
        p_id_specialty => v_id_specialty,
        out_result => v_out_result
    );
    v_result := kabenyk_st.pkg_specialties.is_age_matched (
        p_id_patient => p_id_patient,
        p_id_specialty => v_id_specialty,
        out_result => v_out_result
    );
    v_result := kabenyk_st.pkg_tickets.is_ticket_open (
        p_id_ticket => p_id_ticket,
        out_result => v_out_result
    );
    v_result := kabenyk_st.pkg_tickets.is_time_correct (
        p_id_ticket => p_id_ticket,
        out_result => v_out_result
    );
    v_result := kabenyk_st.pkg_doctors.is_doctor_marked_as_deleted (
        p_id_ticket => p_id_ticket,
        out_result => v_out_result
    );
    v_result := kabenyk_st.pkg_specialties.is_specialty_marked_as_deleted (
        p_id_specialty => v_id_specialty,
        out_result => v_out_result
    );
    v_result := kabenyk_st.pkg_hospitals.is_hospital_marked_as_deleted (
        p_id_ticket => p_id_ticket,
        out_result => v_out_result
    );
    v_result := kabenyk_st.pkg_patients.is_patient_has_oms (
        p_id_patient => p_id_patient,
        out_result => v_out_result
    );
