...
create or replace type kabenyk_st.t_fetch_header as object (
    name varchar2(50),
    value varchar2(100)
);

select utl_http.request('https://www.domain.ru') from dual;

create or replace type kabenyk_st.t_arr_fetch_header as table of kabenyk_st.t_fetch_header;

create or replace function kabenyk_st.http_fetch(
    ...
    p_headers kabenyk_st.t_arr_fetch_header default null, --заголовки запроса
...
begin
    ...
    loop
    declare
        v_header kabenyk_st.t_fetch_header := p_headers(i);
    begin
