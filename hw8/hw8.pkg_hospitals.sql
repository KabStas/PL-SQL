...
as
    ...
    procedure merging_hospitals_data (
        p_hospital in kabenyk_st.t_hospital,
        out_result out integer
    );
...
as
    ...
    begin
        select kabenyk_st.t_hospital(
            ...
            address => h.address,
            external_id_hospital => h.external_id_hospital,
            external_id_town => h.external_id_town
        )
    ...
    begin
        ...
        using (select p_hospital.name,
                      p_hospital.address,
                      p_hospital.external_id_hospital,
                      p_hospital.external_id_town
            from dual) p
        on (h.external_id_hospital = p_hospital.external_id_hospital)
        when matched then update
        set h.name = p_hospital.name,
            h.address = p_hospital.address,
            h.external_id_town = p_hospital.external_id_town
        when not matched then insert (name, address, external_id_hospital, external_id_town)
        values (
            p_hospital.name, p_hospital.address,
            p_hospital.external_id_hospital, p_hospital.external_id_town
        );
